 ...
 *
 * Ten types are provided, covering the three categories required by the
 ...
 * safety-related changes (WEATHER_ALERT, EVACUATE). LOW_STOCK is an
 * event-specific notice unlike the others, it is raised by a
 * Leaf rather than EventControl or an EventGroup, demonstrating that any
 * class can act as a ConcreteSubject, not only composites.
 */
enum NoticeType {
    ...
    LOW_STOCK         ///< A vendor's stock has fallen below its restock threshold.
};
