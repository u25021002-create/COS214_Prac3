...
int main() {
    ...
    // A vehicle waiting in Prep, ready to be released onto the display floor
    // once Task 4.2's runtime reorganisation is demonstrated below.
    VehicleDisplay* podRacer = new VehicleDisplay("Podracer Replica", 25);
    prepZone->add(podRacer);

    // ================== Task 4.4: wire up the original features ==================
    // Feature 1: AutographBooth redirects overflow guests via AnnouncementBooth.
    autographBooth->setOverflowAnnouncer(announcementBooth);
    // Feature 3: DroidShow announces before it resumes.
    droidShow->setAnnouncer(announcementBooth);
    ...
    prepZone->attach(podRacer);

    // Feature 2: MerchStall is its own Subject. PrepZone (as logistics)
    // registers directly with it to receive LOW_STOCK notices -- a
    // registration that has nothing to do with Composite containment,
    // since PrepZone does not contain merchStall.
    merchStall->attach(prepZone);
    ...
    banner("4.2  Runtime reorganisation: PrepZone releases a unit to DisplayZone");
    std::cout << "  Before transfer: PrepZone has " << prepZone->childCount()
              << " child(ren), DisplayZone has " << displayZone->childCount()
              << ".\n";
    prepZone->releaseToDisplay(podRacer, displayZone);
    std::cout << "  After transfer:  PrepZone has " << prepZone->childCount()
              << " child(ren), DisplayZone has " << displayZone->childCount()
              << ".\n";
    std::cout << "  Podracer Replica now hears DisplayZone's notices, not PrepZone's.\n";
    eventControl->issueSafetyNotice(WEATHER_ALERT,
                                    "test: does the transferred unit shelter too?", 1);

    banner("4.4  Feature 1: AutographBooth redirects overflow guests");
    autographBooth->joinQueue(30); // 40 already queued + 30 exceeds capacity of 60

    banner("4.4  Feature 2: MerchStall raises its own LOW_STOCK notice");
    merchStall->sell(85); // drops stock from 100 to 15, below the restock threshold

    banner("3.3  Ordinary operational change: SCHEDULE_CHANGE");
    eventControl->issueNotice(SCHEDULE_CHANGE, "15:30");

    banner("3.3 / 4.3  Condition-based decision: capacity threshold reached?");
    ...
    banner("3.3 / 4.4  RESUME, with the observer attached again "
           "(Feature 3: DroidShow announces before resuming)");
